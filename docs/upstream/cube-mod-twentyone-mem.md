# Upstream packet: `cube-mod-twentyone-mem`
...
Kernel-verified on `main`: `library/Unsorry/CubeModTwentyoneMem.lean` (theorem `cube_mod_twentyone_mem`),
through Gate A (build `--wfail`, axiom audit against the standard whitelist, leanchecker
kernel replay, regenerated ADR-011 binding obligation).

## Proposed contribution

The `git apply`-able new-file diff is at [`cube-mod-twentyone-mem.patch`](cube-mod-twentyone-mem.patch). The target path
`Mathlib/Unsorry/CubeModTwentyoneMem.lean` is a **placeholder** — file placement and the
...
- patterns: `\bcube_mod_twentyone_mem\b`
...
| title | Every cube is congruent to one of {0,1,6,7,8,13,14,15,20} modulo 21. |
...
3. **Raise the draft PR with one command** once you've done 1–2 — from the
   unsorry repo root:
   ```
   python3 -m tools.upstream.raise_pr --goal cube-mod-twentyone-mem --fork <your-github-user> --understood
   ```
