# Upstream packet: `fourth-power-mod-five-mem`
...
Kernel-verified on `main`: `library/Unsorry/FourthPowerModFiveMem.lean` (theorem `fourth_power_mod_five_mem`),
through Gate A (build `--wfail`, axiom audit against the standard whitelist, leanchecker
kernel replay, regenerated ADR-011 binding obligation).

## Proposed contribution

The `git apply`-able new-file diff is at [`fourth-power-mod-five-mem.patch`](fourth-power-mod-five-mem.patch). The target path
`Mathlib/Unsorry/FourthPowerModFiveMem.lean` is a **placeholder** — file placement and the
...
- patterns: `\bfourth_power_mod_five_mem\b`
...
| title | Every natural number's fourth power is congruent to 0 or 1 modulo 5. |
...
3. **Raise the draft PR with one command** once you've done 1–2 — from the
   unsorry repo root:
   ```
   python3 -m tools.upstream.raise_pr --goal fourth-power-mod-five-mem --fork <your-github-user> --understood
   ```
