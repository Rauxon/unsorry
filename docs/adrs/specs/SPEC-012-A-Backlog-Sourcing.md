...
Implements: [ADR-012](../ADR-012-Backlog-Sourcing.md) · Refines: [SPEC-003-A](SPEC-003-A-Goal-Record-Schema.md) · Refined by: [SPEC-033-A](SPEC-033-A-Non-Trivial-Theorem-Enforcement.md) · Status: Living · Updated: 2026-06-14
...
## 3b. Triviality-verify (the second gate, ADR-033)
...
- Advisory-first for one cycle, then blocking. See [SPEC-033-A](SPEC-033-A-Non-Trivial-Theorem-Enforcement.md).
...
- `backlog/<id>.md` — the English statement plus provenance as `- **Field:** value` lines: **Source**, **Reference**, **Absence** (verdict + mathlib rev + date), **Triviality** (verdict + battery version + mathlib rev + date, ADR-033), **Difficulty**. (Gate B does not validate the backlog markdown, so no goal-schema churn; the board reads these.)
