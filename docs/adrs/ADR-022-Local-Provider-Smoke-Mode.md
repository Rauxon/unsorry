...
**we decided for** a `--prove-local --goal <id> --provider <name>` mode that reuses the production proof prompt and full local kernel verification in a preserved detached worktree from local `HEAD`, but exits before every remote and coordination operation; provider execution is adapter-backed, Codex initially joins Claude, and a provider-independent git path guard rejects every edit outside the one target module,
...
**accepting that** the local mode does not test claims, PR creation, or provider-specific decomposition, that its worktree contains committed `HEAD` rather than uncommitted files, and that each provider still needs a live smoke before production enablement.
