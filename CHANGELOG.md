...
- The proof-graph visualiser (`docs/proofs-contributors-visualisation.html`, issue #371) is **restyled to the shared #270 design language** (ADR-038) and is now **mobile-friendly**. `tools/visualiser`'s generated HTML adopts the leaderboard's Tailwind + Inter card UI: the same canvas/card/header treatment, status badges that reuse the diagram's per-status fills, and a mobile-first responsive layout — the diagram and detail panel stack in a single column on small screens and sit side-by-side from `lg`, the header and toolbar wrap, and the Mermaid forest and goals table scroll within the card instead of forcing page-level horizontal scroll. Presentation-only: the embedded graph model, click-to-detail panel, zoom controls, filterable table, and the `--write`/`--check` drift contract (ADR-032) are unchanged, so the post-merge `proofs-visualisation.yml` refresh still works as-is. This closes the last open item on #371 — the design-consistency half of its "use the UX from #270" requirement (the cross-link integration already shipped in 1.12.0).
