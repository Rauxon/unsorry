...
theorem prod_icc_succ_sq_div_k_mul_add_two_telescope (n : ℕ) :
  ...
  | succ n ih =>
    ...
    try have hzb : ((n:ℝ)) ≠ 0 := by exact_mod_cast (by omega : n ≠ 0)
    try have hzc : ((n:ℝ) + 1) ≠ 0 := by positivity
    try have hzd : ((n:ℝ) + 2) ≠ 0 := by positivity
    try have hze : ((n:ℝ) + 3) ≠ 0 := by positivity
    try have hzf : ((n:ℝ) - 1) ≠ 0 := sub_ne_zero.mpr (by exact_mod_cast (by omega : n ≠ 1))
    first
