...
def validate_archive_package(
    ...
    try:
        ...
        # kernel-replayed by Gate A when they were ACTIVE, and the archive
        # byte-identity check (ADR-018 archive-aware immutability, run in
        # gate-a-prepare) proves the archived file is *exactly* that
        # already-verified artifact. Re-running leanchecker on the same immutable
        # proof re-proves nothing — and it loads the package's full mathlib image,
        # which OOM-killed even a 16 GB runner (#764). `lake build --wfail` above
        # stays as packaging sanity (an archive package is a new Lake project, so
        # confirm it still compiles cleanly).
    finally:
        run_step(
